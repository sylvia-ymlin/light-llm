...
def top_k_top_p_filtering(logits: torch.Tensor, top_k: int | None = None, top_p: float | None = None):
    ...
    if top_p is not None and 0 < top_p < 1.0:
        ...
        # filtered.scatter_(1, sorted_idx, sorted_logits) # This was in original, check if correct. 
        # Actually original was:
        # filtered.scatter_(1, sorted_idx, sorted_logits) 
        # But wait, sorted_logits contains the filtered values (with -inf).
        # Yes, recreating the filtered tensor in original order.
        filtered.scatter_(1, sorted_idx, sorted_logits)
